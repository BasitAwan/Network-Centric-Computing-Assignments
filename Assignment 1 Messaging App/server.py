...
def groupMsg(client,userName, members , name):
	...
	or just send a message.""" )
			...
			if gmsg == "*!":
				...
				client.send("Enter file name in directory")

				filename = client.recv(1024)

				client.send("SIZE/")

				filesize = long(client.recv(1024))
				...
				for friend in members:
					...
					if friend == userName:
						...
						thread.start_new_thread(sendFile,(friend, filename))

				gmsg = "I have send a file named : " + filename 
...
def interact(client, userName, reciever):
	...
	if reciever in offline:
		...
		client.send("What message do you want to send?Enter \"**\" or \"*$\"  to stop sending message and go back to the Main menu or to send a file.")
		...
		while mesg != "**":
			...
			if mesg == "*$":
				...
				thread.start_new_thread(sendFile,(reciever, filename))

				print "Done with file " + filename + filesize

				client.send("done with file")

				mesg = "I have send a file named : " + filename 
...
def sendFile(reciever, filename):
	...
	if reciever in online:
		...
		try:
			...
			friend.send("FILE/" + filesize + "/" + filename + "/")

		except:

			del online[reciever]

			sendFile(reciever,filename)

			return

		f = open(filename, 'rb')

		filestosend = f.read(1024)

		friend.send(bytestosend)

		while filetosend != "":

			filetosend = f.read(1024)

			friend.send(filetosend)
